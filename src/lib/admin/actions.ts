...
export async function refreshAction(_prev: AdminState): Promise<AdminState> {
  ...
  if (result.stats.conflicts.length > 0) {
    parts.push(`РАЗМИНАВАНИЯ с ръчни резултати: ${result.stats.conflicts.length}`);
  }
 ...
 *
 * Резултатите, въведени оттук, се маркират като ръчни и обновяването от
 * източника не ги пипа. Часът обаче НЕ е защитен: източникът е графикът и
 * следващото обновяване връща официалния час.
 */
